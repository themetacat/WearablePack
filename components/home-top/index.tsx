...
export default function HomePage({ onClickHandler }: Props, ref) {
    ...
    async (addr, providerCon) => {
      ...
      if (nonceData) {
        ...
        providerCon
          ...
          .then(
            async (signature) => {
              const result = await loginSignature(
                add,
                signature,
                "WearablePack"
              );
  ...
  const abi = [
    ...
    },
    {
      inputs: [
        {
          internalType: "address",
          name: "newERC6551RegistryAddress",
          type: "address",
        },
      ],
      name: "setERC6551RegistryAddress",
      outputs: [],
      stateMutability: "nonpayable",
      type: "function",
    },
  ...
  useEffect(() => {
    if (window.ethereum) {
      // 创建 Web3 实例
      const web3Con = new Web3(
        "https://polygon-mainnet.infura.io/v3/04e6d8eadecd41d68beb8f5e1a57dd7e"
      );
      // const web3Con = new Web3(window.web3.currentProvider);

      const contractAddressCon = "0x7524194dfCf68820006891d5D5810065F233A0B8";
      const daiContract = new web3Con.eth.Contract(
        abi as [],
        contractAddressCon
      );
  ...
  return (
    <Page
      ...
      className={cn(
        "",
        mintContent === true || (showMenu === true && !profile.address)
          ? style.page1
          : style.page
      )}
    >
        ...
        <div className={cn("", style.homeContent)}>
          <div className={cn("", style.homeC)}>
            ...
            <div className={style.iconSvg}>
              ...
              <a href="https://discord.com/invite/yRt6be237P" target="_blank">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
              ...
              <a href="https://twitter.com/Metacat007" target="_blank">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
              ...
              {balanceNum === true ? (
                <div>
                  <p className={style.gasMax}>
                    MATIC Balance: You need {(1 * num).toFixed(2)} MATIC + gas
                    fee <br /> You MATIC wallet Balance: {ethBalance} MATIC
                  </p>
                </div>
              ) : null}
              {/* {switcherNet === true ? (
